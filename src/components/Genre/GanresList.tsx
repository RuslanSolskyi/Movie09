...
const GenresList: React.FC<GenresListProps> = () => {
    ...
    const [page, setPage] = useState<number>(1);
    const [totalPages, setTotalPages] = useState<number>(0);
    const [activeGenre, setActiveGenre] = useState<any | null>(null); // Змінено тип на 'any'
    ...
    const handleGenreClick = (genre: any) => { // Використовуємо тип 'any' для аргументу
        // Знайдемо об'єкт жанру за його назвою
        const selectedGenre = genres.find((g: any) => g.name === genre);

        if (selectedGenre !== undefined) {
            setActiveGenre(selectedGenre); // Змінено на об'єкт жанру
            console.log('Обрано жанр:', genre);
        }
    };

    // Фільтрація фільмів за активним жанром
    const filteredMovies: IMovie[] = activeGenre !== null
        ? movies.filter((movie: IMovie) => movie.genre_ids.includes(activeGenre.id)) // Змінено на activeGenre.id
        : movies;

    console.log('Фільми:', filteredMovies);
            ...
            <div className="genres-container">
                {genres.map((genre: any) => (
                    <GenreBadge
                        ...
                        genre={genre.name}
                        onClick={() => handleGenreClick(genre.name)}
                        isActive={activeGenre !== null && genre.id === activeGenre.id} // Змінено умову
                    />
                ))}
            </div>
            <MovieList movies={filteredMovies} />
